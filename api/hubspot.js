  ...
  try {
    ...
    if (action === 'get_placements') {
      ...
      try {
        ...
        debug.steps.push('deals found: ' + dealIds.length);
      } catch (e) {
        debug.errors.push('deals: ' + e.message);
      }
      ...
      for (const dealId of dealIds) {
        try {
          ...
          debug.steps.push('deal ' + dealId + ' placements: ' + ids.length);
        } catch (e) {
          debug.errors.push('deal assoc: ' + e.message);
        }
      }

      // Step 3: Direct company -> placement orders
      if (allPlacementIds.length === 0) {
        try {
          ...
          allPlacementIds = allPlacementIds.concat(ids);
          debug.steps.push('direct placements: ' + ids.length);
        } catch (e) {
          debug.errors.push('direct: ' + e.message);
        }
      }

      // Deduplicate
      allPlacementIds = [...new Set(allPlacementIds.map(String))];
      debug.steps.push('total unique IDs: ' + allPlacementIds.length);

      // Step 4: Fetch each placement individually
      const data = await fetchPlacementDetails(allPlacementIds);
      debug.steps.push('fetched: ' + data.results.length + ', failed: ' + data.errors.length);

      return res.status(200).json({
        placements: data.results,
